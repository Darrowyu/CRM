...
export interface OperationLog { id: string; user_id?: string; action: string; module: string; target_type?: string; target_id?: string; detail: any; ip_address?: string; created_at: string; operator_name?: string; }
...
export interface SystemHealth { database: { status: string; time: string }; tables: { table_name: string; row_count: number }[]; memory: any; uptime: number; }
